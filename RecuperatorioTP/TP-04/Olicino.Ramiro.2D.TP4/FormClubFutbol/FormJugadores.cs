...
namespace FormClubFutbol
{
    public partial class FormJugadores : Form
    {
        ...
        private void bt_carga_Click(object sender, EventArgs e)
        {
            CargaJugadores();
        }
        ...
        /// <summary>
        /// Carga jugadores desde un archivo Json.
        /// </summary>
        private void CargaJugadores()
        {
            try
            {
                ...
                List<Jugador> jugadores = Archivos<List<Jugador>>.CargarDatosJson(rutaFile);

                jugadores.ForEach((x) => ClubBernaleza.AgregarJugador(x));

                dgv_jugadores.DataSource = ClubBernaleza.ListaJugadoresActivos;

                MessageBox.Show("jUGADORES CARGADOS EXITOSAMENTE", "CORRECTO", MessageBoxButtons.OK);
            }
